...
[ChunkAttributes((uint)ChunkIdentifier.Vector_1D_OF_Channel)]
public class Vector1DOFChannelChunk : ParamChunk
{
    ...
    public Vector1DOFChannelChunk(BinaryReader br) : base((uint)ChunkIdentifier.Vector_1D_OF_Channel)
    {
    ...
    public Vector1DOFChannelChunk(uint version, string param, ushort mapping, Vector3 constants, IList<ushort> frames, IList<float> values) : base((uint)ChunkIdentifier.Vector_1D_OF_Channel)
    {
        Version = version;
        Param = param;
        Mapping = mapping;
